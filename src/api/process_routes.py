...
@ROUTER.post(
    ...
    responses={500: {"model": UnknownError}, 406: {"model": ArgumentError}},
)
...
@ROUTER.post(
    ...
    responses={400: {"model": BadStateError}, 404: {"model": NotFoundError}},
)
async def retry_failed_task(failed_id: UUID) -> RetryResponseModel:
    ...
    if meta := WORKER.backend.get_task_meta(str(failed_id)):
        if meta["status"] == "FAILURE":
            ...
            return RetryResponseModel(
                task_id=result.id, failed_id=failed_id, status=result.state
            )

        errmsg = f"Task ID {failed_id} has the " f"wrong state for a retry (not FAILED)"
        raise HTTPException(status_code=400, detail=errmsg)

    raise HTTPException(status_code=404, detail=f"Task ID {failed_id} does not exist")
...
async def check_task_status(task_id: UUID) -> StatusResponseModel:
    ...
    if not WORKER.backend.get_task_meta(str(task_id)):
        raise HTTPException(status_code=404, detail=f"Task ID {task_id} does not exist")
